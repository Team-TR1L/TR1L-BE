...
-- 2) 무제한 여부 종류
CREATE TABLE IF NOT EXISTS data_billing_type
(
...
-- 7) 요금제 기본 제공 서비스(매핑)
CREATE TABLE IF NOT EXISTS plan_included_option
(
...
-- 10) 유저 직접 구독 부가 서비스
CREATE TABLE IF NOT EXISTS user_option_subscription
(
...
-- 12) 유저 군인 정보
CREATE TABLE IF NOT EXISTS user_soldier
(
    end_date        date,
    start_date      date   NOT NULL,
...
-- 13) 사용자 월별 사용량
CREATE TABLE IF NOT EXISTS monthly_data_usage
(
...
-- =========================================
-- Spring Batch metadata (PostgreSQL)
-- =========================================
...
CREATE INDEX IF NOT EXISTS batch_job_exec_job_inst_idx
    ON batch_job_execution (job_instance_id);

CREATE INDEX IF NOT EXISTS batch_step_exec_job_exec_idx
