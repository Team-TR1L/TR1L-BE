...
public class Step2Processor implements ItemProcessor<BillingTargetKey, WorkDoc> {
    /**
     * @param item
     ...
     */
    @Override
    public Step2WorkDoc process(Step2BillingTargetKey item) throws Exception {
        //insert, update 시간 측정해서 넣어준다.
        Instant now = Instant.now();


        String id = item.billingMonth() + ":" + item.userId();
        return new Step2WorkDoc(
                id,
                item.billingMonth(),
                item.userId(),
                "TARGET",
