...
- 반대로 순간적으로 CPU가 튀어도, 적체가 없다면 확장할 필요가 없음음
...
<img width="7221" height="8192" alt="kafka diagram" src="https://github.com/user-attachments/assets/a83a8753-0c28-48fc-ad3d-e4fe02bd96bb" />

- **Step1: 발송대기 데이터 적재**
  - 보낼 메시지 테이블의 컬럼은 발송 가능 시간(availableTime)과 상태값(READY), s3Url, 유저 PK등이 저장
...
- **Step3 : 관리자 정책 검증**
  ...
  - 이때 보낼 메시지의 상태는 PROCESSING으로 변경
...
- **Step7 : 중복 발송 체크**
  - 실제 발송 전에 보낼 메시지 테이블 조회를 통해 해당 메시지가 이미 발송되었는지 확인
  - 이미 발송된 이력이 있는 경우, 중복 발송을 방지하기 위해 즉시 처리 종료
  - 발송 이력이 없는 경우에만 외부 발송 로직으로 요청을 전달

- **Step8 : 외부 채널 발송**
  - 이때 보낼 메시지의 상태는 SENT로 변경
  - SMS, 이메일 등 외부 전송 채널로 메시지를 발송
  - 이 단계는 외부 시스템에 의존하므로 부분 실패 또는 지연이 발생할 수 있다. (Mock 처리)

- **Step9 : 발송 결과 처리**
  - 발송 성공 시 발송 상태를 SUCCEED로 변경하고 발송 완료 시간을 기록
  - 발송 실패 또는 부분 실패 시에는 실패 로그를 남기며 발송 상태를 FAILED로 변경하고, 인프라 이벤트를 통해 청구서 제작 배치 job으로 전달
