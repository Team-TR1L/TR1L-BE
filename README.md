...
특히 “월 100만 건 이상”의 정산/발송 규모를 전제로 **장애 내성**, **수평 확장**, **비용 최적화**에 초점을 맞추었습니다.
...
- **재처리/재시작의 운영적 불리**
  ...
  - 중복 발송/중복 처리 방지 로직이 시스템 전체에 흩어지기 쉽습니다.
...
- 운영 특성이 다른 컴포넌트는 실행/배포 단위로 분리(`apps/*`)
  ...
  - Consumer는 “대량 발송 처리”를 위해 0~N 수평 확장
...
EC2만으로 이를 운영하면,
- 스케일 아웃을 위해 ASG/Launch Template/AMI 관리 등 운영 구성이 커지고
- **평시에도 인스턴스를 켜두는 비용**이 발생하며
- 스케일링/배포 속도가 느려 슬롯 기반 피크 대응이 불리해집니다.

---

### 선택 기준: 컴포넌트마다 운영 특성이 다르다
우리는 모든 것을 동일한 실행 환경으로 가져가지 않고,  
**역할별 운영 특성에 맞춰 실행 환경을 분리**했습니다.

- **상시 구동 & 트래픽 예측 가능**: `api-server`
- **대량 처리 & 수평 확장이 핵심**: `consumer`
...
- 발송 처리는 외부 호출 지연에 의해 병목이 생기며,
  이 경우 CPU가 낮아도 처리는 느리고 적체는 증가할 수 있습니다.
- 반대로 순간적으로 CPU가 튀어도, 적체가 없다면 확장할 필요가 없습니다.
...
## 메시지 발송(Dispatcher/Consumer) 설계 전략
...
- **Step1: 발송대기 데이터 적재**
  - 생성된 발송 대상 데이터는 발송 가능 시간(availableTime)과 상태값(READY), s3Url, 유저 PK등이 저장
  - 이 시점에서는 실제 발송은 이루어지지 않으며, 발송 스케줄링을 위한 후보군만 관리
  - 이 테이블은 2시간 단위 스케줄러가 조회할 수 있도록 상태값과 발송 가능 시간을 기준으로 인덱싱

- **Step2 : 스케줄러 기반 발송 후보 조회**
  - 전송 책임 서버의 스케줄러가 2시간 주기로 실행
  - 보낼 메시지 테이블에서 상태가 READY이고, 발송 가능 시간이 현재 시간 이전인 데이터만 조회
  - 이를 통해 금지 시간에 해당하지 않는 발송 후보만 선별하며, 한 번에 대량 데이터를 처리하되 중복 조회를 방지

- **Step3 : 관리자 정책 검증**
  - 조회된 발송 후보에 대해 관리자 정책 테이블을 기준으로 정책 검증을 수행
  - 이 단계에서 발송 금지 시간 여부, 허용된 전송 매체 여부를 확인
  - 정책상 발송이 불가능한 경우에는 대기 상태로 유지되며, 정책이 새롭게 추가되거나 전송 매체가 변경된 경우에는 서버 중단 및 재스케줄링 처리

- **Step4 : Kafka 이벤트 발행**
  - 정책 검증을 통과한 발송 대상에 대해서만 Kafka 전송 토픽으로 발송 이벤트를 발행
  - 이 시점부터 발송 책임 서버의 역할은 종료되며, 실제 발송은 컨슈머 서버가 담당
  - Kafka를 통해 발송 요청을 비동기 처리함으로써, 발송 서버 장애와 관계없이 데이터 유실 없이 처리가 가능

- **Step5 : 발송 서버 이벤트 수신**
  - 다수의 발송 서버가 Kafka 컨슈머로 동작하며 발송 이벤트를 수신한다. 각 발송 서버는 이벤트를 수신한 뒤 실제 발송을 위한 전처리를 수행
  - 이 구조를 통해 발송 서버는 수평 확장이 가능하며, 대량 발송 상황에서도 처리량을 안정적으로 유지

- **Step6 : S3 파일 조회 및 복호화**
  - 발송 서버는 이벤트에 포함된 S3 URL과 매체 정보를 복호화한 뒤, S3에서 실제 발송에 필요한 파일을 조회

- **Step7 : 중복 발송 체크**
  - 실제 발송 전에 유저 PK와 상태값을 가지는 테이블 조회를 통해 해당 메시지가 이미 발송되었는지 확인
  - 이미 발송된 이력이 있는 경우, 중복 발송을 방지하기 위해 즉시 처리 종료
  - 발송 이력이 없는 경우에만 외부 발송 로직으로 요청을 전달

- **Step8 : 외부 채널 발송**
  - SMS, 이메일 등 외부 전송 채널로 메시지를 발송
  - 이 단계는 외부 시스템에 의존하므로 부분 실패 또는 지연이 발생할 수 있다. (Mock 처리)

- **Step9 : 발송 결과 처리**
  - 발송 성공 시 발송 상태를 SENT로 변경하고 발송 완료 시간을 기록
  - 발송 실패 또는 부분 실패 시에는 실패 로그를 남기고, 인프라 이벤트를 통해 청구서 제작 배치 잡으로 전달

- **Step10 : 재시도 처리**
  - 재시도 배치 잡은 실패 이벤트를 기반으로 재발송 대상을 선별하여 다시 Step2부터 흐름을 재진입
  - 이를 통해 일시적인 장애 상황에서도 발송 안정성을 확보
